...
// שים לב לשינוי בייבוא - משתמשים ב-SSR עבור דפדפן
...
export default function QuizPage() {
  // יצירת קליינט שמותאם לדפדפן בתוך הקומפוננטה
  ...
  async function saveProgress(finalScore: number) {
    try {
      // כאן הקסם קורה: ה-Browser Client מושך את המשתמש מהסשן האמיתי
      const { data: { session } } = await supabase.auth.getSession();
      const user = session?.user;

      if (!user) {
        console.error('User not identified. Make sure you are logged in.');
        toast.error('Session expired. Please log in again.');
        return;
      }

      console.log('Saving XP for:', user.email);
      ...
      if (!error) {
        toast.success(`Success! +${finalScore} XP added`, { icon: '⚡' });
      } else {
  ...
  const handleAnswer = (selectedId: string) => {
    ...
    setTimeout(() => {
      ...
      if (nextIndex < 10 && nextIndex < questions.length) {
        ...
        if (updatedScore >= 70) fireConfetti();
        // קריאה לפונקציית השמירה עם הניקוד המעודכן
        saveProgress(updatedScore);
      }
    }, isCorrect ? 800 : 1500);
  };

  if (loading) return <div className="flex h-screen items-center justify-center font-bold text-slate-400">Loading Session...</div>;
  ...
  if (gameState === 'finished') return (
    <div className="max-w-2xl mx-auto p-8 flex flex-col items-center min-h-screen bg-slate-50 pt-16" dir="ltr">
      ...
      <button onClick={startNewGame} className="w-full py-5 bg-indigo-600 text-white rounded-2xl font-bold shadow-lg hover:bg-indigo-700 transition-all">
        Start New Quiz
      </button>
    </div>
  );
    ...
    <div className="max-w-4xl mx-auto p-6 flex flex-col items-center justify-center min-h-[90vh]" dir="ltr">
      <div className="w-full max-w-2xl">
        <div className="flex justify-between items-center mb-8 font-bold">
          ...
          <span className="bg-indigo-600 px-6 py-2 rounded-xl text-white shadow-lg">
            {score} XP
          </span>
        </div>
